...
3. Add or update the top entry in `CHANGELOG.md` after the `---` divider using Keep a Changelog format, with the same version and current date (`YYYY-MM-DD`), and include Added/Changed/Fixed subsections that summarize the changes on this branch (cover staged/working copy and recent commits, e.g., use `git diff --stat` and `git log --oneline` for context).
...
- Matching changelog entry documenting the bump.
