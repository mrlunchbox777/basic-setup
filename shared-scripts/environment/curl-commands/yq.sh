...
function get_all_versions {
	if [ "$INCLUDE_PRERELEASE_VERSIONS" == true ]; then
		local all_versions="$(git-github-repo-versions -g "https://github.com/mikefarah/yq" -p v -t)"
	else
		local all_versions="$(git-github-repo-versions -g "https://github.com/mikefarah/yq" -p v -s -t)"
	fi
...
function install_version {
	...
	local filename="yq_${os_string}_${arch_string}"
	local archive_name="${filename}.tar.gz"
	local dirname="${filename}_${TARGET_VERSION}"
	local command_to_run="$(
		cat <<- EOF
			mkdir "$dirname"
			cd "$dirname"
			curl -L -s "https://github.com/mikefarah/yq/releases/download/${TARGET_VERSION}/${archive_name}" -o "$archive_name"
			tar xf "$archive_name"
			chmod +x "./${filename}"
			sudo mv ./${filename} /usr/local/bin/yq
			sudo chown root: /usr/local/bin/yq
