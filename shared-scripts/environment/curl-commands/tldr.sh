...
function get_all_versions {
	if [ "$INCLUDE_PRERELEASE_VERSIONS" == true ]; then
		local all_versions="$(git-github-repo-versions -g "https://github.com/psibi/tldr-hs" -p v -t)"
	else
		local all_versions="$(git-github-repo-versions -g "https://github.com/psibi/tldr-hs" -p v -s -t)"
	fi
