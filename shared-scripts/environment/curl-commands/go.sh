...
function get_installed_version {
	if [ "$(general-command-installed -c go)" == false ]; then
		echo ""
	else
...
function get_all_versions {
	if [ "$INCLUDE_PRERELEASE_VERSIONS" == true ]; then
		local all_versions="$(git-github-repo-versions -g "https://github.com/golang/go" -p go -t)"
	else
		local all_versions="$(git-github-repo-versions -g "https://github.com/golang/go" -p go -s -t)"
	fi
