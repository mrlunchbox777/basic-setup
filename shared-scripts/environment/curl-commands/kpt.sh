...
TARGET_VERSION=""
TEST_VERSION=false
HELP_INSTALL_PAGE="https://kpt.dev/installation/kpt-cli"
...
function get_installed_version {
	if [ "$(general-command-installed -c kpt)" == false ]; then
		echo ""
	else
		echo "v$(kpt version)"
	fi
...
function install_version {
	...
	local filename="kpt"
	local archive_name="kpt_${os_string}_${arch_string}-${version_without_v}.tar.gz"
	local command_to_run="$(
		cat <<- EOF
			mkdir -p /tmp/kpt
			cd /tmp/kpt
			curl -L -s "https://github.com/GoogleContainerTools/kpt/releases/download/${TARGET_VERSION}/${archive_name}" -o "$archive_name"
			tar xf "$archive_name"
			chmod +x "./${filename}"
			sudo mv ./${filename} /usr/local/bin/${filename}
			sudo chown root: /usr/local/bin/${filename}
			rm -rf /tmp/${filename}
		EOF
	)"
	if [ "$FORCE" == true ]; then
