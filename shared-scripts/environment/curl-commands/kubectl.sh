...
function get_installed_version {
	if [ "$(general-command-installed -c kubectl)" == false ]; then
		echo ""
	else
...
function get_all_versions {
	if [ "$INCLUDE_PRERELEASE_VERSIONS" == true ]; then
		local all_versions="$(git-github-repo-versions -g "https://github.com/kubernetes/kubernetes" -p v -t)"
	else
		local all_versions="$(git-github-repo-versions -g "https://github.com/kubernetes/kubernetes" -p v -s -t)"
	fi
