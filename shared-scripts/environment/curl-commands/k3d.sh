...
function get_installed_version {
	if [ "$(general-command-installed -c k3d)" == false ]; then
		echo ""
	else
...
function get_all_versions {
	if [ "$INCLUDE_PRERELEASE_VERSIONS" == true ]; then
		local all_versions="$(git-github-repo-versions -g "https://github.com/k3d-io/k3d" -p v -t)"
	else
		local all_versions="$(git-github-repo-versions -g "https://github.com/k3d-io/k3d" -p v -s -t)"
	fi
