...
TARGET_VERSION=""
TEST_VERSION=false
HELP_INSTALL_PAGE="https://k9scli.io/topics/install/"
...
function get_all_versions {
	if [ "$INCLUDE_PRERELEASE_VERSIONS" == true ]; then
		local all_versions="$(git-github-repo-versions -g "https://github.com/derailed/k9s" -p v -t)"
	else
		local all_versions="$(git-github-repo-versions -g "https://github.com/derailed/k9s" -p v -s -t)"
	fi
...
function install_version {
	...
	local command_to_run="$(
		cat <<- EOF
			mkdir -p /tmp/k9s
			cd /tmp/k9s
			curl -L -s "https://github.com/derailed/k9s/releases/download/${TARGET_VERSION}/${archive_name}" -o "$archive_name"
			tar xf "$archive_name"
			chmod +x "./${filename}"
			sudo mv ./${filename} /usr/local/bin/k9s
