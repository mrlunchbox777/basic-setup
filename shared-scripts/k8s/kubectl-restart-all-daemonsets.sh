...
function help {
	...
	cat <<- EOF
		...
		----------
		description: restart all daemonsets, default all, but can be filtered by label and/or namespace
		----------
		...
		----------
		examples:
		restart all daemonsets - $command_for_help
		----------
	EOF
}
...
bash <(kubectl get daemonset $EXTRA_ARGS -o json | jq -c -r '.items | .[] | "kubectl rollout restart daemonset -n \(.metadata.namespace|@sh) \(.metadata.name|@sh)"')
